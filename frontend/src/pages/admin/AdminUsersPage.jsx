...
import {
  Search, Filter, Users, ShieldCheck, UserCheck, UserX,
  Edit2, Trash2, X, ChevronLeft, ChevronRight, RefreshCw,
  Phone, Mail, Calendar, Shield, User
} from "lucide-react";
...
export default function AdminUsersPage() {
  ...
  const resetFilters = () => {
    setSearchTerm(""); setRoleFilter("all"); setStatusFilter("all"); setPage(1);
  };
  ...
  const confirmDelete = async () => {
    ...
    try {
      setDeleting(true);
      await apiRequest(`/users/${deleteTarget._id}`, { method: "DELETE", token });
  ...
  return (
    <section className="grid gap-6 p-6">
      ...
      <div className="grid grid-cols-2 gap-4 lg:grid-cols-4">
        {[
          { label: "Tổng người dùng", value: stats.total, icon: Users, color: "bg-violet-50 text-violet-600 border-violet-100" },
          { label: "Quản trị viên", value: stats.admins, icon: ShieldCheck, color: "bg-blue-50 text-blue-600 border-blue-100" },
          { label: "Đang hoạt động", value: stats.active, icon: UserCheck, color: "bg-emerald-50 text-emerald-600 border-emerald-100" },
          { label: "Đang bị khóa", value: stats.inactive, icon: UserX, color: "bg-red-50 text-red-600 border-red-100" },
        ].map(({ label, value, icon: Icon, color }) => (
          <div key={label} className={`flex items-center gap-4 rounded-2xl border p-5 bg-white shadow-sm ${color.split(" ")[2]}`}>
            <div className={`grid h-12 w-12 shrink-0 place-items-center rounded-xl ${color.split(" ")[0]} ${color.split(" ")[1]}`}>
      ...
      <section className="rounded-2xl border border-gray-200 bg-white p-5 shadow-sm">
        <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
          <div className="lg:col-span-2">
            <label className="mb-1.5 block text-xs font-semibold text-gray-500">Tìm kiếm</label>
        ...
        <div className="mt-4 flex flex-wrap items-center justify-between gap-3 rounded-xl bg-gray-50 px-4 py-3">
          <span className="text-sm text-gray-600">
            Tìm thấy <span className="font-semibold text-black">{filtered.length}</span> người dùng
          </span>
          <div className="flex gap-2">
            <button onClick={resetFilters} className="rounded-lg border border-gray-300 bg-white px-3 py-2 text-sm font-medium text-gray-700 transition hover:bg-gray-100">
              Xóa bộ lọc
            </button>
            <button onClick={loadUsers} className="flex items-center gap-1.5 rounded-lg border border-gray-300 bg-white px-3 py-2 text-sm font-medium text-gray-700 transition hover:bg-gray-100">
              ...
              {loading ? (
                ...
                paginated.map(user => (
                  <tr key={user._id} className="transition hover:bg-gray-50/70">
                    ...
                    <td className="px-6 py-4">
                      {(() => {
                        const { text, level, date } = formatLastLogin(user.lastLoginAt);
                        const colorMap = {
                          ok: "text-emerald-600",
                        ...
                        return (
                          <span className="flex flex-col gap-0.5">
                            <span className={`text-xs font-medium ${colorMap[level]}`}>{text}</span>
                            {date && level !== "never" && (
        ...
        {!loading && filtered.length > PAGE_SIZE && (
          <div className="flex items-center justify-between border-t border-gray-100 px-6 py-4">
            <span className="text-sm text-gray-500">
              Trang <span className="font-semibold">{currentPage}</span> / {totalPages} &mdash; {filtered.length} người dùng
            </span>
            <div className="flex gap-2">
              <button
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4">
          <div className="w-full max-w-lg rounded-2xl bg-white shadow-2xl">
            <div className="flex items-center justify-between border-b border-gray-100 px-6 py-4">
              ...
              <button onClick={() => setEditUser(null)} className="text-gray-400 transition hover:text-black">
                <X className="h-5 w-5" />
              </button>
            </div>
            ...
            <div className="flex justify-end gap-2 border-t border-gray-100 px-6 py-4">
              <button onClick={() => setEditUser(null)} className="rounded-xl border border-gray-300 px-4 py-2 text-sm font-medium text-gray-700 transition hover:bg-gray-50">
                Hủy
              </button>
              <button
                onClick={saveEdit}
      ...
      {deleteTarget && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4">
          <div className="w-full max-w-md rounded-2xl bg-white shadow-2xl">
            <div className="flex items-center justify-between border-b border-gray-100 px-6 py-4">
              <h3 className="text-sm font-bold text-gray-900">Xác nhận xóa người dùng</h3>
              <button onClick={() => setDeleteTarget(null)} className="text-gray-400 transition hover:text-black">
                <X className="h-5 w-5" />
              </button>
            </div>
            <div className="px-6 py-5">
              ...
              <p className="text-sm text-gray-600">
                Bạn có chắc muốn xóa người dùng này? Hành động này <span className="font-semibold text-red-600">không thể hoàn tác</span>.
              </p>
            </div>
            <div className="flex justify-end gap-2 border-t border-gray-100 px-6 py-4">
              <button onClick={() => setDeleteTarget(null)} className="rounded-xl border border-gray-300 px-4 py-2 text-sm font-medium text-gray-700 transition hover:bg-gray-50">
                Hủy
              </button>
              <button
